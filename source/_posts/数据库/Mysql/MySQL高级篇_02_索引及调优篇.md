...
以图中表为例，如果Col 2 没有索引，我们查找 Col 2 = 89 这条记录，就要逐行去查找、去比较。如果表很大的话，有`上千万条数据`，数据`分布在硬盘不同的位置上面`，摆臂需要前后摆动查询数据，读就意味着要做`很多很多次硬盘I/0`才能找到。最耗时间就是磁盘I/O（涉及到磁盘的旋转时间（速度较快），磁头的寻道时间(速度慢、费时)）

![image-20220616141351236](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616141351236.png)
...
假如给Col2列建立索引，使用 `二叉树` 这样的数据结构，如下图所示

![image-20220616142723266](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616142723266.png)

二叉搜索树的每个结点存储的是 `(K, V) 结构`，key 是 Col 2，value 是该 key 所在行的文件指针（地址）。比如：该二叉搜索树的根节点就是：`(34, 0x07)`。现在对 Col 2 添加了索引，这时再去查找 Col 2 = 89 这条记录的时候会先去查找该二叉搜索树（二叉树的遍历查找）。读到34 ，89 > 34; 向右侧查找，读到89，89==89；找到数据返回。找到之后就根据当前结点的 value 快速定位到要查找的记录对应的地址。我们可以发现，只需要 `查找两次` 就可以定位到记录的地址，查询速度就提高了。

这就是我们为什么要建索引，减少查找次数，减少需要加载的数据， 从而`减少磁盘I/O的次数`，加快查询速率。
...
（1） 提高【where语句】【on语句】【group，order语句】的执行速度。

（2）通过创建唯一索引，可以保证数据库表中每一行 **数据的唯一性** 。 【唯一索引】

### 2.3 缺点 

（1）创建索引和维护索引要 **耗费时间** ，并且随着数据量的增加，所耗费的时间也会增加。 

（2）索引需要占 **磁盘空间** ，如果有大量的索引，索引文件就可能比数据文件更快达到最大文件尺寸。 

（3） **降低更新表的速度** 。当对表 中的数据进行增加、删除和修改的时候，索引也要动态地维护，这样就降低了数据的维护速度。 
...
数据记录在底层是以数据页【16k】为单位存储的。数据记录之间采用单链表数据结构，仅是逻辑上连续。

数据页中有行记录，页目录，槽。槽里是一个包含多条行记录的分组。每个页目录的元素指向一个槽。

页目录是一个数组结构的索引，可以使用二分查找。

先通过页目录找到记录所在的槽，再去该槽读取记录，从而提高查询行记录的速度。

> Mysql为什么使用有序链表而不是数组来存储数据记录？
>
> 如果采用数组结构，物理空间上紧密排列，那么变更数据就会涉及大量的数据迁移，性能低下。
>
> - 数组元素变更可能需要频繁的数据迁移，开销大
> - 需要申请一整块的存储空间来存储所有元素，分配空间成本高
>



> 为什么强烈建议新增数据记录保证**主键递增**？
>
> 因为Mysql是按照主键顺序存储数据记录的，而且每个数据页存储的记录数量有限，主键递增可以减少页分裂的次数。
>
> 新增的数据，通过分配新的数据页来存储。页分裂导致数据迁移，成本高。
...
* 以主键为搜索条件
  ...
  可以在页目录中使用 `二分法` 快速定位到对应的槽，然后再遍历该槽对应分组中的记录即可快速找到指定记录。
...
![image-20220616152453203](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616152453203.png)
...
![image-20241026111506435](MySQL高级篇_02_索引及调优篇_Img/image-20241026111506435.png)

把一些记录放到页里的示意图就是：

![image-20220616152651878](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616152651878.png)
...
![image-20220616153518456](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616153518456.png)
...
![image-20220616155306705](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616155306705.png)
...
![image-20220616160216525](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616160216525.png)
...
![image-20220616160619525](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616160619525.png)
...
![image-20220616160857381](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616160857381.png)
...
![image-20220616162944404](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616162944404.png)
...
![image-20220616171135082](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616171135082.png)
...
![image-20220616173512780](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616173512780.png)

如图，我们生成了一个存储更高级目录项的 页33 ，这个页中的两条记录分别代表页30和页32，如果用 户记录的主键值在 [1, 320) 之间，则到页30中查找更详细的目录项记录，如果主键值 不小于320 的 话，就到页32中查找更详细的目录项记录。

我们可以用下边这个图来描述它：

![image-20220616173717538](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616173717538.png)
...
![image-20220616203852043](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616203852043.png)
...
![image-20220616213109383](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616213109383.png)
...
![image-20220616215251172](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220616215251172.png)
...
![image-20220617151918786](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617151918786.png)

如果二级索引中目录项记录的内容只是 `索引列 + 页号` 的搭配的话，那么为 `c2` 列建立索引后的B+树应该长这样：

![image-20220617152906690](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617152906690.png)
...
![image-20220617154135258](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617154135258.png)
...
![image-20220617160413479](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617160413479.png)
...
![image-20220617160625006](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617160625006.png)
...
![image-20220617161151125](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617161151125.png)
...
![image-20220617162153587](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617162153587.png)
...
![image-20220617162604272](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617162604272.png)

上图中哈希函数h有可能将两个不同的关键字映射到相同的位置，这叫做 碰撞 ，在数据库中一般采用 链 接法 来解决。在链接法中，将散列到同一槽位的元素放在一个链表中，如下图所示：

![image-20220617162703006](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617162703006.png)
...
![image-20220617163657697](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617163657697.png)
...
![image-20220617164022728](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617164022728.png)

但是特殊情况，就是有时候二叉树的深度非常大，比如：

![image-20220617164053134](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617164053134.png)
...
![image-20220617165105005](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617165105005.png)
...
![image-20220617165124685](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617165124685.png)
...
![image-20220617165937875](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617165937875.png)
...
![image-20220617170526488](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617170526488.png)

### 6.6 B+Tree

MySQL官网说明：

![image-20220617170710329](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617170710329.png)
...
![image-20240601211116026](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20240601211116026.png)
...
![image-20220617175516191](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220617175516191.png)
...
![image-20220324200116204](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220324200116204.png)
...
![image-20220324200502569](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220324200502569.png)
...
![image-20220324200934245](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220324200934245.png)
...
![image-20220324201808920](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220324201808920.png)
...
![image-20220324224809508](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220324224809508.png)
...
h=![img](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/format,png.png)
...
![image-20220325104247259](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220325104247259.png)
...
![image-20220325104508228](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220325104508228.png)
...
![image-20220325170858020](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220325170858020.png)
...
![image-20220325175937136](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220325175937136.png)
...
![image-20220325180029352](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220325180029352.png)
...
![image-20220325181405473](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220325181405473.png)
...
![image-20220325181420603](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220325181420603.png)
...
![image-20220325181712981](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220325181712981.png)
...
![image-20220325194823764](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220325194823764.png)
...
![image-20220325195048342](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220325195048342.png)
...
![image-20220326113218648](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326113218648.png)
...
![image-20220326120805996](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326120805996.png)
...
![image-20220326121611806](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326121611806.png)
...
![image-20220326122616487](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326122616487.png)
...
![image-20220326122717663](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326122717663.png)
...
![image-20220326122751920](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326122751920.png)
...
![image-20220326122857145](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326122857145.png)
...
![image-20220326123056983](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326123056983.png)
...
![image-20220326123147690](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326123147690.png)
...
- `PRIMARY` 与 `UNION`与 `UNION RESULT`
  ...
  ![image-20220326125611904](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326125611904.png)
  ...
  ![image-20220326125627303](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326125627303.png)
...
- `DEPENDENT SUBQUERY`
  ...
  ![image-20220326130111650](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326130111650.png)
...
- `DEPENDENT UNION`
  ...
  ![image-20220326130433291](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326130433291.png)
...
- `DERIVED`
  ...
  ![image-20220326141653065](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326141653065.png)
...
- `MATERIALIZED`
  ...
  ![image-20220326142034981](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326142034981.png)
...
![image-20220325201510359](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220325201510359.png)
...
![image-20220326170537304](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326170537304.png)
...
![image-20220326171219278](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326171219278.png)
...
![image-20220326172022150](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326172022150.png)
...
![image-20220326172126970](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326172126970.png)
...
![image-20220326172256011](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326172256011.png)
...
![image-20220326172802426](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326172802426.png)
...
![image-20220326173156258](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326173156258.png)
...
![image-20220326173338273](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326173338273.png)
...
![image-20220326173520015](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326173520015.png)
...
- `Using index condition`
  ...
  ![image-20220326173622086](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326173622086.png)
...
- `Using join buffer (Block Nested Loop)`  
  ...
  ![image-20220326173721122](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326173721122.png)
...
- `Not exists`
  ...
  ![image-20220326182016458](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326182016458.png)
...
- `Using intersect(...) 、 Using union(...) 和 Using sort_union(...)`  
  ...
  ![image-20220326182155207](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326182155207.png)
...
- `Using temporary`  
  ...
  ![image-20220326183048710](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326183048710.png)
  ...
  ![image-20220326183135116](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326183135116.png)
...
- EXPLAIN的Column与JSON的对应关系:(来源于MySQL 5.7文档)
  ...
  ![image-20220326184629857](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326184629857.png)
...
![image-20220326185603021](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326185603021.png)
...
![image-20220326190931718](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326190931718.png)

![image-20220326191114795](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326191114795.png)
...
![image-20220326224834648](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326224834648.png)
...
![image-20220326225238412](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326225238412.png)

如果此时再插入一条主键值为 `9` 的记录，那它插入的位置就如下图：

![image-20220326225248650](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326225248650.png)
...
- student表的字段stuno上设置有索引  
  ...
  ![image-20220326225939241](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326225939241.png)
...
- 索引优化生效(没有计算)：  
  ...
  ![image-20220326230203661](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326230203661.png)
...
![image-20220326234953835](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220326234953835.png)
...
- 查看索引是否失效
  ...
  ![image-20220327000012828](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327000012828.png)
...
![image-20220327000138701](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327000138701.png)
...
![image-20220327000222914](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327000222914.png)
...
**练习**:假设:index(a,b,c)
![image-20220327001211718](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327001211718.png)
...
![image-20220327105415185](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327105415185.png)
...
![image-20220327105619638](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327105619638.png)
...
![image-20220327110003998](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327110003998.png)
...
![image-20220327110048502](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327110048502.png)
...
![image-20220327112700993](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327112700993.png)
...
![image-20220327110542488](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327110542488.png)
...
![image-20220327110718552](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327110718552.png)
...
![image-20220327111114145](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327111114145.png)
...
- 对于内连接来说:
  ...
  - 对于外连接来说:
    ...
    **测试1结果：**
    ...
    ![image-20220327113715776](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327113715776.png)

    得出这种结论太不可思议了，跟上一个show warnings 看看：

    ![image-20220327114615193](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327114615193.png)

    

    **测试2结果：**

    ![image-20220327113840201](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327113840201.png)

    继续show warnings \G

    ![image-20220327114721018](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327114721018.png)
...
![image-20220327115112379](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327115112379.png)
...
![image-20220327115215270](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327115215270.png)
...
![image-20220327115921235](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327115921235.png)



驱动表中的每条记录通过被驱动表的索引进行访问，因为索引查询的成本是比较固定的，故mysql优化器都倾向于使用记录数少的表作为驱动表(外表)。

![image-20220327120030338](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327120030338.png)
...
![image-20220327143958188](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327143958188.png)



![image-20220327144009591](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327144009591.png)
...
![image-20220327151158056](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327151158056.png)
...
- Hash Join是做`大数据集连接`时的常用方式，优化器使用两个表中较小(相对较小)的表利用Join Key在内存中建立`散列表`，然后扫描较大的表并探测散列表，找出与Hash表匹配的行。
  ...
  ![image-20220327151646951](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327151646951.png)
...
![image-20220327154029455](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327154029455.png)
...
![image-20220327154234022](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327154234022.png)
...
![image-20220327154426669](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327154426669.png)
...
![image-20220327154631330](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327154631330.png)
...
![image-20220327163331675](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327163331675.png)
...
![image-20220327163436260](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327163436260.png)
...
![image-20220327170746020](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327170746020.png)
...
![image-20220327171114961](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327171114961.png)
...
![image-20220327171516492](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327171516492.png)
...
![image-20220327181204713](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327181204713.png)
**优化思路二**(几乎没法用)
...
![image-20220327181228362](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327181228362.png)
...
![image-20220327194911745](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327194911745.png)
...
![image-20220327195102695](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327195102695.png)
...
![image-20220327195221475](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327195221475.png)
...
![image-20220327195413811](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327195413811.png)
...
![image-20220327195610323](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327195610323.png)
...
![image-20220327195812263](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327195812263.png)
...
![image-20220327181808091](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327181808091.png)

以及  

![image-20220327181820437](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327181820437.png)
...
![image-20220327201222126](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327201222126.png)
...
![image-20220327212419933](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327212419933.png)
...
![image-20220327214304560](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327214304560.png)





![image-20220327214423178](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327214423178.png)
...
![image-20220327181853000](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327181853000.png)

![image-20220327181901279](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327181901279.png)
...
![image-20220327181910781](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327181910781.png)

![image-20220327181918603](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327181918603.png)
...
![image-20220327194004090](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327194004090.png)
...
![image-20220327194010973](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327194010973.png)
...
![image-20220327194022473](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327194022473.png)
...
![image-20220327194036197](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327194036197.png)
...
![image-20220327194047194](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327194047194.png)
...
![image-20220327194059890](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327194059890.png)
...
![image-20220328095515382](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220328095515382.png)

该表不符合1NF，因为规则说“表的每个属性必须具有原子(单个）值”lisi和zhaoliu员工的emp_mobile值违反了该规则(有两个电话)。为了使表符合1NF，我们应该有如下表数据:

![image-20220328095522609](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220328095522609.png)
...
![image-20220328101954268](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220328101954268.png)
...
![image-20220328102007725](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220328102007725.png)
...
![image-20220328102411308](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220328102411308.png)
...
![image-20220328102401313](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220328102401313.png)
...
![image-20220328102856949](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220328102856949.png)



商品信息表：  

![image-20220328102906913](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220328102906913.png)

新的商品流水表如下所示：

![image-20220328102921528](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220328102921528.png)

举例4：
课程评论表 class_comment ，对应的字段名称及含义如下：

![image-20220328102934457](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220328102934457.png)

学生表 student ，对应的字段名称及含义如下：

![image-20220328102948234](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220328102948234.png)
...
![image-20220328102757397](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220328102757397.png)
...
![image-20220327232334207](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327232334207.png)

![image-20220327232344144](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327232344144.png)
...
![image-20220327232559837](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327232559837.png)

![image-20220327232655999](http://fastly.jsdelivr.net/gh/lowols/Pictures@main/posts/MySQL高级篇_02_索引及调优篇_Img/image-20220327232655999.png)
